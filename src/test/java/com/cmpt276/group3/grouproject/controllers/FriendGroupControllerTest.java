...
public class FriendGroupControllerTest {
        ...
        void saveQuestionnaire_ignoresUnknownDropdownValues() throws Exception {
                ...
                verify(friendGroupService).savePreference(eq(mike),
                                argThat((GroupPreference preference) -> preference.getVibe() == null
                                                && preference.getCampus() == null));
        }
        ...
        void changeRole_rejectsAnUnknownRoleWithoutCallingTheService() throws Exception {
                ...
                verify(friendGroupService, never()).changeRole(any(User.class), anyLong(), anyLong(),
                                any(GroupRole.class));
        }
        ...
        @Test
        void viewGroup_redirectsPendingRegularMember() throws Exception {
                loggedIn();

                GroupMembership pendingMembership = new GroupMembership(group, mike, GroupRole.PENDING);

                when(friendGroupService.findGroup(10L)).thenReturn(group);
                when(friendGroupService.getMemberships(group))
                                .thenReturn(new ArrayList<>(
                                                Arrays.asList(pendingMembership)));

                mockMvc.perform(get("/groups/10").session(session))
                                .andExpect(status().is3xxRedirection())
                                .andExpect(redirectedUrl("/"));

                verify(friendGroupService, never()).isMember(group, mike);
                verify(friendGroupService, never()).isAdmin(group, mike);
        }

        @Test
        void viewGroup_allowsPendingSitewideModerator() throws Exception {
                ...
                GroupMembership pendingMembership = new GroupMembership(group, moderator, GroupRole.PENDING);
                ...
                when(friendGroupService.isMember(group, moderator))
                                .thenReturn(true);

                mockMvc.perform(get("/groups/10").session(session))
                                .andExpect(status().isOk())
                                .andExpect(view().name("group"))
                                .andExpect(model().attribute("currentUser", moderator))
                                .andExpect(model().attribute("group", group))
                                .andExpect(model().attribute("memberships",
                                                Arrays.asList(pendingMembership)))
                                .andExpect(model().attribute("memberCount", 1))
                                .andExpect(model().attribute("isMember", true))
                                .andExpect(model().attribute("isApprovedMember", false))
                                .andExpect(model().attribute("isGroupAdmin", true));

                /*
                 * The controller grants site-wide MOD/ADMIN users group-admin
                 * privileges directly, so it does not call the service's isAdmin().
                 */
                verify(friendGroupService, never())
                                .isAdmin(group, moderator);
        }

        @Test
        void viewGroup_allowsPendingSitewideAdmin() throws Exception {
                User administrator = new User(
                                ...
                                Gender.NONE,
                                "");
                administrator.setId(100L);

                GroupMembership pendingMembership = new GroupMembership(group, administrator, GroupRole.PENDING);

                when(auth.isLoggedIn(session)).thenReturn(true);
                when(auth.getUser(session)).thenReturn(administrator);
                ...
                when(friendGroupService.isMember(group, administrator))
                                .thenReturn(true);
        ...
        @Test
        void myGroups_includesPendingMemberships() throws Exception {
                loggedIn();

                GroupMembership pendingMembership = new GroupMembership(group, mike, GroupRole.PENDING);

                when(friendGroupService.getPreference(mike))
                                .thenReturn(completePreference());

                when(friendGroupService.getMyMemberships(mike))
                                .thenReturn(new ArrayList<>(
                                                Arrays.asList(pendingMembership)));

                when(friendGroupService.getMemberCount(group))
                                .thenReturn(3L);
                ...
                verify(friendGroupService).getMemberCount(group);
        }
}
